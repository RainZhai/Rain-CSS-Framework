       ...
       */
			...
			_transOptionToLi : function() {
				$.each(selectobj.find("option"), function(i, elem) {
					...
					lis += '<li class="oh" val="' + $(elem).attr('value') + '" text= "' + $(elem).text()
							+ '"><a class="ggrey btn greybtn block tdn p tac posr" href="javascript:;">'
							+ '<span class="iconbtn-right ib hs micon tac posa icon-ok-sign icon-white"></span>' + $(elem).text()
							+ '</a></li>';
				});
				o.setUlobj(lis);
				return o;
			...
			_parseData : function(data) {
				...
				for ( var i = 0, l = d.length; i < l; i++) {
					...
					lis += '<li val="' + d[i].value + '" text= "' + d[i].name
							+ '"><a class="ggrey btn greybtn block tdn p tac posr" href="javascript:;">'
							+ '<span class="iconbtn-right ib hs micon tac posa icon-ok-sign icon-white"></span>' + d[i].name
							+ '</a></li>';
				}
			...
			buildHtml : function() {
				var _html = '<i class="icon-chevron-down posa iconbtn-right"></i>'
						+ '<span class="j_innerbtn paddingL tac toe block oh wsn fs-1"></span>' + '<select></select>';
