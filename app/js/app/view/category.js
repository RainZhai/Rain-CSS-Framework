		 ...
		 '{{each categorylist as value i}} '+
			'<div class="listitem wf-100 h-7 clearfix bgw so mts rounds">'+
			        '<div class="l fl wf-40 fullh">'+
				        '<a class="block tdn oh hf-80 ps" href="{{value.url}}" target="_blank"><img src="{{value.imgurl}}" alt="{{value.name}}" height="100%"/></a>'+
				'</div>'+
				'<div class="l fl wf-50 fullh">'+
					'<a class="block tal tdn fs14px mts pls" href="{{value.url}}" target="_blank">{{value.name}}</a>'+
					'<p class="oh h-4 pls">{{value.intro}}</p>'+
				'</div>'+
